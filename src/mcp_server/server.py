...
def build_registry() -> ToolRegistry:
    ...
    eval_suite = SafetyEvalSuite.from_config(settings.evals())
    interp_suite = InterpretabilitySuite.from_config(settings.interpretability())
    gov_cfg = settings.governance()
    governance = GovernanceRetriever(kb_client, gov_cfg.doc_path) if gov_cfg.doc_path else None
    return ToolRegistry(kb=kb_client, evals=eval_suite, interp=interp_suite, governance=governance)
...
async def run_server() -> None:
    ...
    async def _call_tool(tool_name: str, arguments: Dict[str, Any]):
        return await registry.dispatch(tool_name, arguments)
